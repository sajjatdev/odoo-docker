...
class Users(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Qplexity (updates, module installation, ...)'))
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Qplexity portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Qplexity is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Qplexity in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.Qplexity.com/documentation/master/administration/install/deploy.html#https for details.",
                    source
                )
