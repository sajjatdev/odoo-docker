...
# here to init the T1 fonts cache at the start-up of Qplexity so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Qplexity start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Qplexity with at least two workers to print a pdf version of the reports.')
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Qplexity with at least two workers to convert images to html.')
    else:
...
class IrActionsReport(models.Model):
    ...
    def _handle_merge_pdfs_error(self, error=None, error_stream=None):
        raise UserError(_("Qplexity is unable to merge the generated PDFs."))
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('Qplexity is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,
