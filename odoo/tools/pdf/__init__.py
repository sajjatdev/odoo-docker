...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with Qplexity's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Qplexity's logo in the banner.
...
class QplexityPdfFileReader(PdfFileReader):
...
class QplexityPdfFileWriter(PdfFileWriter):
