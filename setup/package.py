...
class QplexityTestTimeoutError(Exception):
    pass


class QplexityTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise QplexityTestError("Installation of package failed")
        else:
        ...
        raise QplexityTestError("Package test: FAILED. Not able to install base.")
...
class Docker():
    ...
    def test_odoo(self):
        logging.info('Starting to test Qplexity install test')
        ...
        if self.is_running():
            self.stop()
            raise QplexityTestTimeoutError('Qplexity pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise QplexityTestError('Error while installing/starting Qplexity after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
