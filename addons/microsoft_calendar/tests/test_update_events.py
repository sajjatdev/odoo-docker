...
class TestUpdateEvents(TestCommon):
    ...
    # -------------------------------------------------------------------------------
    # Update from Qplexity to Outlook
    # -------------------------------------------------------------------------------
    ...
    def test_update_odoo_simple_event_without_sync(self, mock_patch):
        """
        Update an Qplexity event without Outlook sync enabled
        """
    ...
    def test_update_simple_event_from_odoo(self, mock_patch):
        """
        Update an Qplexity event with Outlook sync enabled
        """
    ...
    def test_update_simple_event_from_odoo_attendee_calendar(self, mock_patch):
        """
        Update an Qplexity event from the attendee calendar.
        """
    ...
    def test_update_name_of_one_event_of_recurrence_from_odoo(self, mock_patch):
        """
        Update one Qplexity event name from a recurrence from the organizer calendar.
        """
    ...
    def test_update_start_of_one_event_of_recurrence_from_odoo(self, mock_patch):
        """
        Update one Qplexity event start date from a recurrence from the organizer calendar.
        """
        if not self.sync_odoo_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    def test_update_start_of_one_event_of_recurrence_from_odoo_with_overlap(self, mock_patch):
        """
        Update one Qplexity event start date from a recurrence from the organizer calendar, in order to
    ...
    def test_update_name_of_one_event_of_recurrence_from_odoo_attendee_calendar(self, mock_patch):
        """
        Update one Qplexity event name from a recurrence from the atendee calendar.
        """
        if not self.sync_odoo_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    def test_update_name_of_one_and_future_events_of_recurrence_from_odoo(
        ...
        """
        Update a Qplexity event name and future events from a recurrence from the organizer calendar.
        """
        if not self.sync_odoo_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    def test_update_start_of_one_and_future_events_of_recurrence_from_odoo(
        ...
        """
        Update a Qplexity event start date and future events from a recurrence from the organizer calendar.
        """
        if not self.sync_odoo_recurrences_with_outlook_feature():
            return
    ...
    def test_update_one_and_future_events_of_recurrence_from_odoo_attendee_calendar(
        ...
        """
        Update a Qplexity event name and future events from a recurrence from the attendee calendar.
        """
        if not self.sync_odoo_recurrences_with_outlook_feature():
            return
        # arrange
    ...
    # -------------------------------------------------------------------------------
    # Update from Outlook to Qplexity
    # -------------------------------------------------------------------------------
    ...
    def test_forbid_simple_event_become_recurrence_sync_on(self, mock_patch):
        """
        Forbid in Qplexity simple event becoming a recurrence when Outlook Calendar sync is active.
        """
    ...
    def test_update_synced_event_with_sync_config_paused(self, mock_patch):
        ...
        # Try to update a simple event in Qplexity Calendar.
    ...
    def test_changing_event_organizer_to_another_user(self, mock_insert, mock_delete, mock_get_events):
        """
        Allow editing the event organizer to another user only if the proposed organizer have its Qplexity Calendar synced.
    ...
    def test_restart_sync_with_synced_recurrence(self, mock_patch):
        """ Ensure that sync restart is not blocked when there are recurrence outliers in Qplexity database. """
    ...
    def test_update_old_event_synced_with_outlook(self, mock_get_events, mock_write_from_microsoft):
        """
        There are old events in Qplexity which share the same state with Microsoft and get updated (without changes) in Qplexity
        due to a few seconds of update time difference, triggering lots of unwanted spam for attendees on Microsoft side.
        Don't update old events in Qplexity if update time difference between Microsoft and Qplexity is not significant.
        """
        ...
        # Mock the modification time in Microsoft with 10 minutes ahead Qplexity event 'write_date'.
        # Synchronize Microsoft Calendar and ensure that the skipped event was not updated in Qplexity.
        mock_get_events.return_value = (
            MicrosoftEvent([dict(
                self.simple_event_from_outlook_organizer,
