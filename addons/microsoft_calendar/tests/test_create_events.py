...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Qplexity event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Qplexity organizer calendar.
        """
    ...
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_odoo(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Qplexity attendee calendar.
        There is a Qplexity user that matches with the organizer email address.
        """
    ...
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_odoo(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Qplexity attendee calendar.
        no Qplexity user that matches with the organizer email address.
        """
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@Qplexity.com", 'name': "John Doe"},
        })
    ...
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Qplexity attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Qplexity.
        """
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Qplexity recurrent event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_recurrent_event_with_sync(self, mock_insert, mock_get_events):
        """
        A Qplexity recurrent event is created when Outlook sync is enabled.
        """
        if not self.sync_odoo_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A Qplexity recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Qplexity organizer calendar.
        """
    ...
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Qplexity attendee calendar.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Qplexity due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Qplexity Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Qplexity Calendar synced.
        ...
        # In Qplexity, this event will be created (behind the screens) by a synced Qplexity user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Qplexity through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Qplexity (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Qplexity.
        """
    ...
    def test_create_event_with_default_and_undefined_sensitivity(self):
        """ Check if microsoft events are created in Qplexity when 'None' sensitivity setting is defined and also when it is not. """
        # Sync events from Microsoft to Qplexity after adding the sensitivity (privacy) property.
        ...
        # Ensure that synced events have the correct privacy field in Qplexity.
    ...
    def test_create_videocall_sync_microsoft_calendar(self, mock_insert, mock_get_events):
        """
        Test syncing an event from Qplexity to Microsoft Calendar.
        ...
        # Verify the event's videocall_location is updated in Qplexity
    ...
    def test_skip_sync_for_non_synchronized_users_new_events(self, mock_insert):
        ...
        Qplexity, as well cause problems in the future the synchronization of that event for the original owner.
        """
        ...
        self.assertTrue(event, "The event for the not synchronized owner must be created in Qplexity.")
...
class TestSyncQplexity2MicrosoftMail(TestCommon, MailCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
