...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by Qplexity recordset, one instance can be a single Microsoft event or a
    ...
    def _meta_odoo_id(self, microsoft_guid):
        """Returns the Qplexity id stored in the Microsoft Event metadata.
    ...
    def odoo_ids(self):
        """
        Get the list of Qplexity event ids already mapped with Outlook events (self)
        """
        return tuple(e._odoo_id for e in self if e._odoo_id)

    def _load_odoo_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing Qplexity events:
        1) extract unmapped events only,
        2) match Qplexity events and Outlook events which have both a ICalUId set,
        ...
        # 1. try to match unmapped events with Qplexity events using their iCalUId
        ...
        # 2. try to match unmapped events with Qplexity events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        ...
        for ms_event in unmapped_events:
            ...
            if odoo_event:
                ...
                # don't forget to also set the global event ID on the Qplexity event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current Qplexity user is the organizer of the event according to Outlook event, so return his id.
        2) the current Qplexity user is NOT the organizer and:
           2.1) we are able to find a Qplexity user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a Qplexity user matching the organizer email address and we return False, meaning
                that no Qplexity user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        ...
        if organizer_email:
            # Warning: In Microsoft: 1 email = 1 user; but in Qplexity several users might have the same email
    ...
    def match_with_odoo_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing Qplexity events, and return the list of matched events
        """
