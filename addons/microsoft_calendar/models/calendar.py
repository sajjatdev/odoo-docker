...
class Meeting(models.Model):
    ...
    def create(self, vals_list):
        ...
        # Forbid recurrence creation in Qplexity, suggest its creation in Outlook due to the spam limitation.
            ...
            if not sender_sync_status and current_sync_status:
                raise ValidationError(
                    _("For having a different organizer in your event, it is necessary that "
                      "the organizer have its Qplexity Calendar synced with Outlook Calendar."))
    ...
    def _forbid_recurrence_update(self):
        ...
        if any(not record.ms_universal_event_id for record in self):
            # If any event is not synced, suggest deleting it in Qplexity and recreating it in Outlook.
            error_msg = _(
                "Due to an Outlook Calendar limitation, recurrence updates must be done directly in Outlook Calendar.\n"
                "If this recurrence is not shown in Outlook Calendar, you must delete it in Qplexity Calendar and recreate it in Outlook Calendar.")
    ...
    def write(self, values):
        ...
        # Forbid recurrence updates through Qplexity and suggest user to update it in Outlook.
    ...
    def _get_microsoft_sync_domain(self):
        ...
        # Define 'custom_lower_bound_range' param for limiting old events updates in Qplexity and avoid spam on Microsoft.
    ...
    def _ensure_attendees_have_email(self):
        ...
        if invalid_event_ids:
            ...
            raise ValidationError(_("For a correct synchronization between Qplexity and Outlook Calendar, "
    ...
    def _cancel_microsoft(self):
        ...
        There are 2 cases:
          1) the organizer is an Qplexity user: he's the only one able to delete the Qplexity event. Attendees can just decline.
          2) the organizer is NOT an Qplexity user: any attendee should remove the Qplexity event.
        """
        user = self.env.user
