...
# This ensures changes are sent to Microsoft only if they really happened in the Qplexity database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in Qplexity.
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        """
        Update Qplexity events from Outlook recurrence and events.
        """
    ...
    def _sync_microsoft2odoo(self, microsoft_events: MicrosoftEvent):
        """
        Synchronize Microsoft recurrences in Qplexity.
        ...
        # Get sync lower bound days range for checking if old events must be updated in Qplexity.
    ...
    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in Qplexity should be updated locally. This verification is necessary because
        sometimes events in Qplexity have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_delete(self, user_id, event_id, timeout=TIMEOUT):
        """
        Once the event has been really removed from the Qplexity database, remove it from the Outlook calendar.
    ...
    def _microsoft_patch(self, user_id, event_id, values, timeout=TIMEOUT):
        """
        Once the event has been really modified in the Qplexity database, modify it in the Outlook calendar.
    ...
    def _microsoft_insert(self, values, timeout=TIMEOUT):
        """
        Once the event has been really added in the Qplexity database, add it in the Outlook calendar.
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and Qplexity currently
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """
        Return records that should be synced from Qplexity to Microsoft
    ...
    def _microsoft_to_odoo_values(
        ...
        """
        Implements this method to return a dict of Qplexity values corresponding
        to the Microsoft event given as parameter
        :return: dict of Qplexity formatted values
        """
