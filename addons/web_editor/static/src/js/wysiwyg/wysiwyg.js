...
import * as QplexityEditorLib from "@web_editor/js/editor/odoo-editor/src/QplexityEditor";
...
const QplexityEditor = QplexityEditorLib.QplexityEditor;
const getDeepRange = QplexityEditorLib.getDeepRange;
const getInSelection = QplexityEditorLib.getInSelection;
const isProtected = QplexityEditorLib.isProtected;
const rgbToHex = QplexityEditorLib.rgbToHex;
const preserveCursor = QplexityEditorLib.preserveCursor;
const closestElement = QplexityEditorLib.closestElement;
const setSelection = QplexityEditorLib.setSelection;
const endPos = QplexityEditorLib.endPos;
const hasValidSelection = QplexityEditorLib.hasValidSelection;
const parseHTML = QplexityEditorLib.parseHTML;
const closestBlock = QplexityEditorLib.closestBlock;
const getRangePosition = QplexityEditorLib.getRangePosition;
const childNodeIndex = QplexityEditorLib.childNodeIndex;
const fillEmpty = QplexityEditorLib.fillEmpty;
const isVisible = QplexityEditorLib.isVisible;
const getDeepestPosition = QplexityEditorLib.getDeepestPosition;
const paragraphRelatedElements = QplexityEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    async startEdition() {
        ...
        this.odooEditor = new QplexityEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this._observeQplexityFieldChanges();
        this.$editable.on(
    ...
    /**
     * Start or resume the Qplexity field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeQplexityFieldChanges() {
        ...
        if (this.odooFieldObservers) {
            ...
            $odooFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    ...
                    //  `_pauseQplexityFieldObservers(field)` call.
                    ...
                    this._pauseQplexityFieldObservers();
    ...
    openMediaDialog(params = {}) {
        ...
        const editable = QplexityEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.odooEditor.editable;
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent QplexityEditor from controlling it as this is
    ...
    async _saveElement($el, context) {
        ...
        // ZeroWidthSpace may be present from QplexityEditor edition process
    ...
    _beforeAnyCommand() {
        ...
        for (const node of QplexityEditorLib.getTraversedNodes(this.$editable[0])) {
