...
export function usePartnerAutocomplete() {
    const keepLastQplexity = new KeepLast();
    ...
    async function autocomplete(value) {
        ...
        return new Promise((resolve, reject) => {
            const odooPromise = getQplexitySuggestions(value, isVAT).then((suggestions) => {
            ...
            const concatResults = () => {
                // Add Clearbit result with Qplexity result (with unique domain)
    ...
    /**
     * Use Qplexity Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getQplexitySuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastQplexity.add(prom);
