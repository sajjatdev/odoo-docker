...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/qplexity?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/qplexity/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
