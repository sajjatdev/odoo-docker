...
def check_certificate():
    ...
    if cn == 'QplexityTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Qplexity DB and
    ...
    try:
        ...
        if response.status == 200:
            ...
            _logger.info(
                "Current IoT Box local git branch: %s / Associated Qplexity database's git branch: %s",
                local_branch,
                db_branch,
            )
...
def check_image():
    ...
    """
    url = 'https://nightly.Qplexity.com/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_odoo_server_url():
    """Get the URL of the linked Qplexity database.
    If the IoT Box is in access point mode, it will return ``None`` to avoid
    connecting to the server.

    :return: The URL of the linked Qplexity database.
    :rtype: str or None
    """
...
def load_certificate():
    """
    Send a request to Qplexity with customer db_uuid and enterprise_code to get a true certificate
    """
    ...
    url = 'https://www.Qplexity.com/odoo-enterprise/iot/x509'
    data = {
        'params': {
            'db_uuid': db_uuid,
            'enterprise_code': enterprise_code
    ...
    try:
        ...
        _logger.exception("An error occurred while trying to reach Qplexity.com servers.")
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Qplexity server
    """
    server = get_odoo_server_url()
    if server:
        urllib3.disable_warnings()
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Qplexity\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Qplexity\\downloaded_file.zip', 'C:\\Program Files\\Qplexity\\new_folder'))
