...
echo "
odoo_help() {
  echo 'Welcome to Qplexity IoTBox tools'
  echo 'odoo                Starts/Restarts Qplexity server manually (not through odoo.service)'
  echo 'odoo_logs           Displays Qplexity server logs in real time'
  echo 'odoo_conf           Displays Qplexity configuration file content'
  ...
  echo 'odoo_start          Starts Qplexity service'
  echo 'odoo_stop           Stops Qplexity service'
  echo 'odoo_restart        Restarts Qplexity service'
  echo 'odoo_dev <branch>   Resets Qplexity on the specified branch from odoo-dev repository'
}
...
# This will be modified by a unique password on the first start of Qplexity
...
wget 'https://nightly.Qplexity.com/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.Qplexity.com/master/iotbox/eftapi.so' -P /usr/lib/
