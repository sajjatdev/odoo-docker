...
describe("link preview", () => {
    ...
    test("test external link preview", async () => {
        onRpc("/html_editor/link_preview_external", () => {
            return {
                og_description:
                    "From ERP to CRM, eCommerce and CMS. Download Qplexity or use it in the cloud. Grow Your Business.",
                og_image: "https://www.Qplexity.com/web/image/41207129-1abe7a15/homepage-seo.png",
                og_title: "Open Source ERP and CRM | Qplexity",
                og_type: "website",
                og_site_name: "Qplexity",
                source_url: "http://Qplexity.com/",
            };
        });
        const { editor } = await setupEditor(`<p>[]</p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill("http://Qplexity.com/");
        await animationFrame();
        expect(".o_we_replace_title_btn").toHaveCount(1);
        expect(".o_extra_info_card").toHaveCount(1);
        expect(".o_we_url_link").toHaveText("Open Source ERP and CRM | Qplexity");
