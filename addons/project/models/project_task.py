...
class Task(models.Model):
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Qplexity-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Qplexity-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Qplexity-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
