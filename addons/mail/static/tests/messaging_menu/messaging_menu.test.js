...
test("rendering with QplexityBot has a request (default)", async () => {
    ...
    await contains(".o-mail-NotificationItem", { text: "QplexityBot has a request" });
});

test("rendering without QplexityBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
test("rendering without QplexityBot has a request (accepted)", async () => {
...
test("respond to notification prompt (denied)", async () => {
    ...
    await contains(".o_notification:has(.o_notification_bar.bg-warning)", {
        text: "Qplexity will not send notifications on this device.",
    });
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("respond to notification prompt (granted)", async () => {
    ...
    await contains(".o_notification:has(.o_notification_bar.bg-success)", {
        text: "Qplexity will send notifications on this device!",
    });
});

test("no 'QplexityBot has a request' in mobile app", async () => {
    patchBrowserNotification("default");
    // simulate Android Qplexity App
    mockUserAgent("Chrome/0.0.0 Android (QplexityMobile; Linux; Android 13; Qplexity TestSuite)");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("rendering with PWA installation request", async () => {
    ...
    await contains(".o-mail-NotificationItem-name", { text: "QplexityBot has a suggestion" });
    await contains(".o-mail-NotificationItem-text", {
        text: "Come here often? Install Qplexity on your device!",
    });
...
test("chat preview should not display correspondent name in body", async () => {
    ...
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Demo", email: "demo@Qplexity.com" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
        channel_type: "chat",
        channel_member_ids: [
