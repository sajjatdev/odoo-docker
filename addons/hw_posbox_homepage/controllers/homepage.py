...
class IotBoxOwlHomePage(Home):
    ...
    def odoo_service_restart(self):
        ...
        return json.dumps({
            'status': 'success',
            'message': 'Qplexity service restarted',
        })
    ...
    def upgrade_iotbox(self):
        ...
        return json.dumps({
            'title': "Qplexity's IoTBox - Software Upgrade",
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "Qplexity's IoT Box - Handlers list",
    ...
    def update_wifi(self, essid, password, persistent=False):
        ...
        res_payload = {
            ...
            'server': {
                ...
                'message': 'Redirect to Qplexity Server' if server else 'Redirect to IoT Box'
            }
        }
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) Qplexity's configuration and logger to the given logger_name to the given level.
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Qplexity Iot logger given an IoT handler name
