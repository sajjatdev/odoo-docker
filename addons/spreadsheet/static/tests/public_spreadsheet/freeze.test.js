...
import { freezeQplexityData } from "@spreadsheet/helpers/model";
import { QplexityPivot, QplexityPivotRuntimeDefinition } from "@spreadsheet/pivot/odoo_pivot";
...
test("odoo pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeQplexityData(model);
...
test("odoo links are replaced with their label", async function () {
        ...
        sheets: [
            {
                cells: {
                    ...
                    A4: { content: "[external_link](https://Qplexity.com)" },
    ...
    const frozenData = await freezeQplexityData(model);
    ...
    expect(frozenData.sheets[0].cells.A4.content).toBe("[external_link](https://Qplexity.com)");
