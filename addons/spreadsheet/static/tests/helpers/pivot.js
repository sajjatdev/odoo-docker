...
import { QplexityPivot } from "@spreadsheet/pivot/odoo_pivot";
...
export async function insertPivotInSpreadsheet(model, pivotId, params) {
    ...
    if (!(ds instanceof QplexityPivot)) {
        throw new Error("The pivot data source is not an QplexityPivot");
    }
    await ds.load();
 ...
 * @returns {Promise<{ model: QplexitySpreadsheetModel, env: object, pivotId: string}>}
 */
