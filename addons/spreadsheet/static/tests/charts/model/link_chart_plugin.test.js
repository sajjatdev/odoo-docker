...
test("Links between charts and ir.menus are correctly imported/exported", async function () {
    ...
    expect(exportedData.chartQplexityMenusReferences[chartId]).toBe(1, {
        message: "Link to odoo menu is exported",
    });
    const importedModel = new Model(exportedData, { custom: { env } });
    const chartMenu = importedModel.getters.getChartQplexityMenu(chartId);
...
test("Can undo-redo a LINK_ODOO_MENU_TO_CHART", async function () {
    ...
    expect(model.getters.getChartQplexityMenu(chartId).id).toBe(1);
    model.dispatch("REQUEST_UNDO");
    expect(model.getters.getChartQplexityMenu(chartId)).toBe(undefined);
...
test("Links of Qplexity charts are duplicated when duplicating a sheet", async function () {
    ...
    expect(model.getters.getChartQplexityMenu(newChartId)).toEqual(
        model.getters.getChartQplexityMenu(chartId)
    );
});
