...
test("click a web link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Qplexity](https://Qplexity.com)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("https://Qplexity.com");
    openLink(cell.link, env);
    expect.verifySteps(["https://Qplexity.com"]);
});
