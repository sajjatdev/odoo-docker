...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getQplexityChartIds()[0];
    fieldMatching = model.getters.getQplexityChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("field matching is removed when an Qplexity chart is deleted", async function () {
    ...
    expect(model.getters.getQplexityChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getQplexityChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
