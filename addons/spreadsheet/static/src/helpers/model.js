...
import { QplexitySpreadsheetModel } from "@spreadsheet/model";
import { QplexityDataProvider } from "@spreadsheet/data_sources/odoo_data_provider";
...
/**
 * @typedef {import("@spreadsheet").QplexitySpreadsheetModel} QplexitySpreadsheetModel
 */
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const odooDataProvider = new QplexityDataProvider(env);
    const model = new QplexitySpreadsheetModel(data, { custom: { odooDataProvider } }, revisions);
    return model;
}

/**
 * @param {QplexitySpreadsheetModel} model
 */
export async function waitForQplexitySources(model) {
...
export async function waitForDataLoaded(model) {
    await waitForQplexitySources(model);
...
function containsLinkToQplexity(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeQplexityData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, cell] of Object.entries(sheet.cells)) {
            ...
            if (containsQplexityFunction(cell.content)) {
            ...
            if (containsLinkToQplexity(evaluatedCell.link)) {
 ...
 * @returns {boolean}
 */
function containsQplexityFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
