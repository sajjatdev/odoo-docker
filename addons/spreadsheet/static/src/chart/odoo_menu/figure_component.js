...
patch(spreadsheet.components.FigureComponent.prototype, {
    ...
    async navigateToQplexityMenu() {
        const menu = this.env.model.getters.getChartQplexityMenu(this.props.figure.id);
        if (!menu) {
    ...
    get hasQplexityMenu() {
        return this.env.model.getters.getChartQplexityMenu(this.props.figure.id) !== undefined;
    },
    async onClick() {
        if (this.env.isDashboard() && this.hasQplexityMenu) {
            this.navigateToQplexityMenu();
        }
    },
});
