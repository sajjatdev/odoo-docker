...
import { QplexityCorePlugin } from "@spreadsheet/plugins";
...
const CHART_PLACEHOLDER_DISPLAY_NAME = {
    odoo_bar: _t("Qplexity Bar Chart"),
    odoo_line: _t("Qplexity Line Chart"),
    odoo_pie: _t("Qplexity Pie Chart"),
};

export class QplexityChartCorePlugin extends QplexityCorePlugin {
    static getters = /** @type {const} */ ([
        "getQplexityChartIds",
        "getChartFieldMatch",
        "getQplexityChartDisplayName",
        "getQplexityChartFieldMatching",
    ]);
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            getIds: () => this.getters.getQplexityChartIds(),
            getDisplayName: (chartId) => this.getters.getQplexityChartDisplayName(chartId),
            getFieldMatching: (chartId, filterId) =>
                this.getQplexityChartFieldMatching(chartId, filterId),
            getModel: (chartId) =>
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "EDIT_GLOBAL_FILTER":
                if (cmd.chart) {
                    this._setQplexityChartFieldMatching(cmd.filter.id, cmd.chart);
                }
                break;
        }
    }
            ...
            if (sheet.figures) {
                for (const figure of sheet.figures) {
                    if (figure.tag === "chart" && figure.data.type.startsWith("odoo_")) {
                        this._addQplexityChart(figure.id, figure.data.fieldMatching);
                    }
                }
            }
        }
     ...
     */
    getQplexityChartFieldMatching(chartId, filterId) {
     ...
     */
    _setQplexityChartFieldMatching(filterId, chartFieldMatches) {
        const charts = { ...this.charts };
     ...
     */
    _addQplexityChart(chartId, fieldMatching = undefined) {
        const charts = { ...this.charts };
        if (!fieldMatching) {
