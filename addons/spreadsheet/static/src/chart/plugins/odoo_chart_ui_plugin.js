...
import { QplexityUIPlugin } from "@spreadsheet/plugins";
import { navigateTo } from "../../actions/helpers";

export class QplexityChartUIPlugin extends QplexityUIPlugin {
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getQplexityChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getQplexityChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getQplexityChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getQplexityChartDataSourceId(chartId);
        return this.charts[dataSourceId];
    }

    /**
     * Get the callback used for onClick and onHover in an Qplexity Chart
     */
     ...
     * @return {Promise[]}
     */
    _getQplexityChartsWaitForReady() {
        return this.getters
            .getQplexityChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getQplexityChartDataSourceId(chartId) {
     ...
     */
    _refreshQplexityCharts() {
        for (const chartId of this.getters.getQplexityChartIds()) {
            this._refreshQplexityChart(chartId);
        }
    }
}
