...
/** Plugin that link charts with Qplexity menus. It can contain either the Id of the odoo menu, or its xml id. */
export class ChartQplexityMenuPlugin extends QplexityCorePlugin {
    static getters = /** @type {const} */ (["getChartQplexityMenu"]);
    constructor(config) {
        super(config);
     ...
     */
    getChartQplexityMenu(chartId) {
        const menuId = this.odooMenuReference[chartId];
        return menuId ? this.getters.getIrMenu(menuId) : undefined;
    }

    import(data) {
        if (data.chartQplexityMenusReferences) {
            this.odooMenuReference = data.chartQplexityMenusReferences;
        }
    }

    export(data) {
        data.chartQplexityMenusReferences = this.odooMenuReference;
    }
}
