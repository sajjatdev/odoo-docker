...
class TestWebPushNotification(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['mail.push.device'].sudo().create([
            {
                'endpoint': f'https://test.Qplexity.com/webpush/user{(idx + 1)}',
    ...
    def test_notify_by_push(self, push_to_end_point):
        """ When posting a comment, notify both inbox and people outside of Qplexity
        aka email """
        ...
        for channel, sender, notification_count in zip(
            ...
            with self.subTest(channel_type=channel.channel_type):
                ...
                if notification_count > 0:
                    ...
                    self.assertEqual(push_to_end_point.call_args.kwargs['device']['endpoint'], 'https://test.Qplexity.com/webpush/user2')
    ...
    def test_notify_by_push_channel_with_channel_notifications_settings(self, push_to_end_point):
        ...
        self.env["mail.push.device"].sudo().create(
            [
                {
                    "endpoint": f"https://test.Qplexity.com/webpush/user{(idx + 20)}",
                    "expiration_time": None,
                    "keys": json.dumps(
                        {
                            "p256dh": "BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A",
        ...
        self.assertEqual(push_to_end_point.call_args.kwargs["device"]["endpoint"], "https://test.Qplexity.com/webpush/user20")
        push_to_end_point.reset_mock()
        ...
        self.assertEqual(push_to_end_point.call_args_list[0].kwargs["device"]["endpoint"], "https://test.Qplexity.com/webpush/user20")
        self.assertEqual(push_to_end_point.call_args_list[1].kwargs["device"]["endpoint"], "https://test.Qplexity.com/webpush/user21")
        push_to_end_point.reset_mock()
    ...
    def test_push_notifications_cron(self, push_to_end_point):
        ...
        for index in range(10, 14):
            self.env['mail.push.device'].sudo().create([{
                'endpoint': 'https://test.Qplexity.com/webpush/user%d' % index,
                'expiration_time': None,
                'keys': json.dumps({
                    'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                    'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    def test_push_notifications_error_device_unreachable(self, post):
        ...
        notification_count = self.env['mail.push.device'].search_count([('endpoint', '=', 'https://test.Qplexity.com/webpush/user2')])
    ...
    def test_push_notifications_error_encryption_simple(self, post):
        ...
        self._assert_notification_count_for_cron(0)
        post.assert_called_once()
        self.assertEqual(post.call_args.args[0], 'https://test.Qplexity.com/webpush/user2')
    ...
    def test_push_notification_regenerate_vapid_keys(self):
        ...
        with self.assertRaises(InvalidVapidError):
            self.env['mail.push.device'].register_devices(
                endpoint='https://test.Qplexity.com/webpush/user1',
                expiration_time=None,
                keys=json.dumps({
                    'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                    'auth': 'DJFdtAgZwrT6yYkUMgUqow'
