...
@tagged("odoobot")
class TestQplexitybot(MailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestQplexitybot, cls).setUpClass()
        ...
        cls.odoobot_ping_body = f'<a href="http://Qplexity.com/odoo/res.partner/{cls.odoobot.id}" class="o_mail_redirect" data-oe-id="{cls.odoobot.id}" data-oe-model="res.partner" target="_blank">@QplexityBot</a>'
    ...
    def test_odoobot_ping(self):
        ...
        # Qplexitybot should not be a follower but user_employee and user_admin should
