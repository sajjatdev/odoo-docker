...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "qplexity";
...
export function urlToState(urlObj) {
    ...
    if (prefix === "qplexity" || isScopedApp()) {
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/qplexity") &&
            (["/web", "/qplexity"].includes(url.pathname) || url.pathname.startsWith("/qplexity/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/qplexity") && url.hash) {
