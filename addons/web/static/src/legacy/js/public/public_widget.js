...
function QplexityEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
QplexityEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
QplexityEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Qplexity where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new QplexityEvent(this, name, info);
