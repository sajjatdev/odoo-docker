...
test("in form view", async () => {
    Contact._records = [{ id: 1, email: "john.doe@Qplexity.com" }];
    await mountView({
        type: "form",
        resModel: "contact",
        resId: 1,
    ...
    expect(`.o_field_email input[type="email"]`).toHaveValue("john.doe@Qplexity.com");
    expect(`.o_field_email a`).toHaveCount(1);
    expect(`.o_field_email a`).toHaveAttribute("href", "mailto:john.doe@Qplexity.com");
    expect(`.o_field_email a`).toHaveAttribute("target", "_blank");
    await fieldInput("email").edit("new@Qplexity.com");
    expect(`.o_field_email input[type="email"]`).toHaveValue("new@Qplexity.com");
});

test("in editable list view", async () => {
    Contact._records = [
        { id: 1, email: "john.doe@Qplexity.com" },
        { id: 2, email: "jane.doe@Qplexity.com" },
    ];
    await mountView({
        type: "list",
        resModel: "contact",
    ...
    expect(queryAllTexts(`tbody td:not(.o_list_record_selector) a`)).toEqual([
        "john.doe@Qplexity.com",
        "jane.doe@Qplexity.com",
    ]);
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:john.doe@Qplexity.com");
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:new@Qplexity.com");
});
