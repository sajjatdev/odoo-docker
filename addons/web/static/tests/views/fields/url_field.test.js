...
test("UrlField in form view", async () => {
    ...
    await fieldInput("url").edit("https://www.Qplexity.com");
    expect(`.o_field_widget input[type="text"]`).toHaveValue("https://www.Qplexity.com");
});
...
test("onchange scenario", async () => {
    ...
    Product._records = [{ id: 1, url: "Qplexity.com", url_source: "another.com" }];
    await mountView({
        type: "form",
        resModel: "product",
        resId: 1,
    ...
    expect(".o_field_widget[name=url]").toHaveText("Qplexity.com");
