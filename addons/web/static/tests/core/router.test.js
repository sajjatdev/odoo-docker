...
describe("stateToUrl", () => {
    test("encodes URI compatible strings", (assert) => {
        ...
        expect(stateToUrl({ a: "11", b: "summer wine" })).toBe("/qplexity?a=11&b=summer%20wine");
        expect(stateToUrl({ b: "2", c: "", e: "kloug,gloubi" })).toBe(
            "/qplexity?b=2&c=&e=kloug%2Cgloubi"
        );
    });
...
describe("urlToState", () => {
    test("deserialize queryString", () => {
        expect(_urlToState("/qplexity?a=11&g=summer%20wine")).toEqual({
            a: 11,
            g: "summer wine",
        });
        expect(_urlToState("/qplexity?g=2&c=&e=kloug%2Cgloubi")).toEqual({
            g: 2,
            c: "",
            e: "kloug,gloubi",
        });
...
describe("pushState", () => {
    ...
    test("can hide keys", async () => {
        ...
        await tick();
        expect(router.current).toEqual({ k1: 2, k2: 3 });
        expect(browser.location.href).toBe("https://www.hoot.test/qplexity?k2=3");
    });
    test("different order of keys shouldn't push a new state", async () => {
        redirect("/qplexity?k1=2");
        createRouter({
