...
class TestSyncQplexity2Google(TestSyncGoogle):
    ...
    def test_update_synced_event_with_sync_config_paused(self):
        ...
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertEqual(record.name, "Updated Event", "Assert that event name was updated in Qplexity Calendar")
        self.assertTrue(record.need_sync, "Sync variable must be true for updating event when sync re-activates")
    ...
    def test_delete_synced_event_with_sync_config_paused(self):
        """
        Deletes a synced event with synchronization paused, event must be archived in Qplexity and
        ...
        # Assert that synchronization is paused, delete wasn't called and record was archived in Qplexity.
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertFalse(record.active, "Event must be archived in Qplexity after unlinking it")
...
@tagged('odoo2google')
class TestSyncQplexity2GoogleMail(TestTokenAccess, TestSyncGoogle, MailCommon):
