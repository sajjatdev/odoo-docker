...
# This ensures changes are sent to Google only if they really happened in the Qplexity database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Qplexity.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Qplexity DB (_sync_google2odoo).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_google2odoo(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in Qplexity. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Qplexity
        :param write_dates: A dictionary mapping Qplexity record IDs to their write dates.
        ...
        # We have to check if an event already exists in Qplexity.
        # Explanation:
        ...
        for gevent in pending:
            ...
            # Use the record's write_date to apply Google updates only if they are newer than Qplexity's write_date.
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Qplexity to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Qplexity to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
