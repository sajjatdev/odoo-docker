...
class GoogleEvent(abc.Set):
    ...
    Inspired by Qplexity recordset, one instance can be a single Google event or a
    ...
    def _meta_odoo_id(self, dbname):
        """Returns the Qplexity id stored in the Google Event metadata.
    ...
    def _load_odoo_ids_from_metadata(self, env, model):
        ...
        # Hence, we may have two Google recurrences linked to the same Qplexity id.
        # Therefore, we only consider Qplexity records without google id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Qplexity.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Qplexity (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Qplexity) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Qplexity several users might have the same email :/
